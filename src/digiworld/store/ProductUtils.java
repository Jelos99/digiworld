...
public class ProductUtils {
	...
	public static boolean addNewProduct(Product product) throws SQLException
	{
		String QUERY = "INSERT INTO products" +
				"(name, service, description, price, discount, region, is_dlc, category) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public static boolean updateProduct(int product_ID, Product newProduct) throws SQLException 
	{
		...
		String QUERY = "UPDATE products" 
				+ "SET name = '?', service = '?', description = '?', price = '?',  discount = '?', region = '?', is_dlc = '?', category = '?'"
				+ "WHERE product_id = ?";
		...
		preparedStatement.setString(8, newProduct.getCategory());
		preparedStatement.setInt(9, product_ID);
	...
	/* Crea una lista di prodotti a partire dalla categoria
	 *
	 */
	public static ArrayList<Product> retriveProductsByCategory(String category) throws SQLException
	{
